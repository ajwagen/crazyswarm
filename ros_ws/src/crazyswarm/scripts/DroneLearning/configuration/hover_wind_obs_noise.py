...
sim_config = SimConfiguration(
    ...
    obs_noise=ConfigValue[float](default=0.01, randomize=False)
)

env_config = AdaptationConfiguration(
    include = [EnvCondition.WIND]
)

config = AllConfig(drone_config, wind_config, init_config, sim_config, env_config)
