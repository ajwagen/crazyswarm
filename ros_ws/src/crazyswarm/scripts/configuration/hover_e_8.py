...
drone_config = DroneConfiguration(
    mass = ConfigValue[float](1.0, randomize=False, min=0.75, max=1.5),
    I = ConfigValue[float](1.0, randomize=True, min=0.5, max=2.0),
...
sim_config = SimConfiguration(
    ...
    latency=ConfigValue[int](default=0, randomize=False, min=0.015, max=0.025),
    k=ConfigValue[float](default=1, randomize=False)
)

adapt_config = AdaptationConfiguration(
    include = [EnvCondition.I]
)
